...
from bookapi.extensions import pwd_context, jwt, apispec
from bookapi.auth.helpers import revoke_token, is_token_revoked, add_token_to_database
...
def register():
  ...
  if username is None or password is None or email is None or confirm_password is None:
    ret = {error:'username, email, password are required'}
    return jsonify(ret), 400

  if ' ' in username:
    ret = {error:'Username should not contain spaces'}
    return jsonify(ret), 400
  ...
  if user is not None:
    ret = {error:'A user with that name already exists'}
    return jsonify(ret), 400

  user = User(username=username, email=email)
  user.set_password(password)
  user.save()
  return jsonify({"msg": "Registered Success"}), 200
